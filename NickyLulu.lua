...
function Lulu:LogicE()
    if self.AutoEShild then
		...
		for i,hero in pairs(GetAllyHeroes()) do
			if hero ~= nil then
				ally = GetAIHero(hero)
				if not ally.IsMe and not ally.IsDead and GetDistance(ally.Addr) < self.E.range and GetDistance(Enemy) < 1000 then
					if self.AutoEShild then
...
function Lulu:LogicW()
    if self.AutoWShild then
		...
		for i,hero in pairs(GetAllyHeroes()) do
			if hero ~= nil then
				ally = GetAIHero(hero)
				if not ally.IsMe and not ally.IsDead and GetDistance(ally.Addr) < self.W.range and GetDistance(Enemy) < 1000 then
					if self.AutoWShild then
						if CountBuffByType(ally.Addr, 5) > 0 or CountBuffByType(ally.Addr, 5) > 0 then
...
function Lulu:CastR()
    ...
    for i,hero in pairs(GetAllyHeroes()) do
        if hero ~= 0 then
            ally = GetAIHero(hero)
            if not ally.IsMe and not ally.IsDead and GetDistance(ally.Addr) < self.R.range and GetDistance(Enemy) < 1000 and CountEnemyChampAroundObject(Enemy, self.R.range) < self.UseRange then
...
function Lulu:CastRIsMy()
    local UseR = GetTargetSelector(900)
    Enemy = GetAIHero(UseR)
	if GetPercentHP(myHero.Addr) <= self.UseRmy and IsValidTarget(Enemy, 1000) and CountEnemyChampAroundObject(Enemy, self.R.range) < self.UseRange then
		if CanCast(_R) then
