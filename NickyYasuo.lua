...
function Yasuo:OnProcessSpell(unit, spell)
    ...
    if self.CanItem and CountBuffByType(target.Addr, 29) > 0 or CountBuffByType(target.Addr, 30) > 0 and (DGW + DGQ + DGE > target.HP) then
            CastSpellTarget(myHero.Addr, _R) 
            end 
        end 
    end 
    if spell and unit.IsEnemy and target and target.Id == spell.TargetId then 
    ...
    if unit.IsEnemy and unit.Type == 0 then
        if target and target.Id == spell.TargetId then
            if IsValidTarget(target , self.E.Range) and CanCast(_W) then
...
function Yasuo:OnUpdateBuff(unit, buff)
    ...
    if self.CanItem and self.CCType[buff.Type] then
        for i, enemys in pairs(self:GetEnemyHeroes()) do
            target = GetAIHero(enemys)
                if target ~= 0 and (DGW + DGQ + DGE > target.HP) then 
...
function Yasuo:LC()
    if self.LE then
        for i ,P in pairs(self:EnemyMinionsTbl()) do
            if P ~= 0 then
                minion = GetUnit(P)
                if GetDistance(minion) < self.E.Range and not self:IsUnderTurretEnemy(P) then
                    CastSpellTarget(P.Addr, _E)
                end 
            end 
        end 
